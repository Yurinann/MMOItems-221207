...
public class UpgradeTemplate {
    ...
    public UpgradeTemplate(@NotNull ConfigurationSection config) {
        ...
        for (String key : config.getKeys(false)) {
            ...
            if (stat == null) {
                ffp.log(FriendlyFeedbackCategory.ERROR, "Stat '$r{0}$b' $fnot found$b.", statFormat);
                continue;
            }
            if (!(stat instanceof Upgradable)) {
                ffp.log(FriendlyFeedbackCategory.ERROR, "Stat $r{0}$b is $fnot upgradeable$b.", stat.getId());
                continue;
            }
            if (!(stat.getClearStatData() instanceof Mergeable)) {
                ffp.log(FriendlyFeedbackCategory.ERROR, "Stat Data used by $r{0}$b is $fnot mergeable$b, and thus it cannot be upgradeable. Contact the dev of this ItemStat.", stat.getId());
                continue;
            }
    ...
    public void upgradeTo(@NotNull MMOItem mmoitem, int level) {
        ...
        if (mmoitem.hasData(ItemStats.UPGRADE)) {
            ...
            dat = new UpgradeData(null, null, false, false, 0, 0, 100);
        }
        dat.setLevel(level);
