...
public class RecipeBrowserGUI extends EditionInventory {
     ...
     */
    @NotNull
    final ItemStack nextPage = ItemFactory.of(Material.ARROW).name(FFPMMOItems.get().getExampleFormat() + "Next Page").build();
    @NotNull
    final ItemStack prevPage = ItemFactory.of(Material.ARROW).name(FFPMMOItems.get().getExampleFormat() + "Previous Page").build();
    @NotNull
    final ItemStack noRecipe = ItemFactory.of(Material.LIGHT_GRAY_STAINED_GLASS_PANE).name("").build();
    /**
    ...
    void arrangeInventory(@NotNull Inventory inv) {
        ...
        if (registeredRecipes.size() >= ((currentPage + 1) * 21)) {
            inv.setItem(36, nextPage);
        }

        // Well order them I guess
        HashMap<Integer, RecipeRegistry> reg = new HashMap<>();
        int op = 0;
