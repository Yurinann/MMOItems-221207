...
import io.lumine.mythic.lib.adventure.text.event.ClickEvent;
import io.lumine.mythic.lib.adventure.text.format.NamedTextColor;
import io.lumine.mythic.lib.adventure.text.format.TextDecoration;
...
import net.Indyuce.mmoitems.util.AdventureUtils;
...
public class MaterialStat extends ItemStat<MaterialData, MaterialData> {
    ...
    public void whenInput(@NotNull EditionInventory inv, @NotNull String message, Object... info) {
        ...
        EnumUtils.getIfPresent(Material.class, message.toUpperCase().replace("-", "_").replace(" ", "_"))
                .ifPresentOrElse(material -> {
                    inv.getEditedSection().set("material", material.name());
                    inv.registerTemplateEdition();
                    player.sendMessage(MMOItems.plugin.getPrefix() + "Material successfully changed to %s.".formatted(material.name()));
                }, () -> AdventureUtils.message(player, Component.text("Invalid material!", NamedTextColor.RED)
                        .append(Component.text("(Click for a list of valid materials)", NamedTextColor.AQUA, TextDecoration.BOLD)
    ...
    public void whenDisplayed(List<String> lore, Optional<MaterialData> statData) {
        lore.add(ChatColor.GRAY + "Current Value: "
                + (statData.map(materialData -> ChatColor.GREEN + MMOUtils.caseOnWords(materialData.getMaterial().name().toLowerCase().replace("_", " ")))
                .orElseGet(() -> ChatColor.RED + "None")));
